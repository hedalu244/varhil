...
限定詞のグロスはラベルの代わりに番号を使う。新規限定詞のグロスは`DET番号+`、継承限定詞のグロスは`DET番号`、終了限定詞のグロスは`DET番号-`とする。また、孤立限定詞のグロスは`DET`とする。
...
**前置詞** は格を指定して述語句に名詞句を付加する品詞である。`e` で終わる機能語が前置詞であり、`e` 以前の部分は格に対応する。
...
前置詞のグロスは`//PRE`の後ろに格を表す訳語等を続けた形とする。
...
文の解釈が最後まで終わっても述語が受けるすべての格に引数が割り当てられていなかったときは、その場限りの変数（つまり孤立限定詞）を補って解釈する。

従って、前置詞の第一引数が孤立限定詞であるとき、その前置詞と孤立限定詞は省略できる。
...
**関係詞** は格を指定して名詞句に述語句を付加する品詞である。 `ei` で終わる機能語が関係詞であり、`ei` 以前の部分は格に対応する。
...
関係詞のグロスは`//REL`の後ろに格を表す訳語等を続けた形とする。
...
無標格を表す関係詞が二番目の引数に孤立限定詞を取っている場合（ある述語句が無標格関係詞と孤立限定詞に挟まれている場合）、さらにその名詞句が単文になっていないとき、その関係詞と限定詞は省略できる。つまり、無標格を取る述語句は単独で名詞句になれる。この名詞句や名詞句になれる述語を便宜上 **名詞** と呼ぶ。
...
文を `nou`と`noi`で挟むことで、その文が表す命題の否定命題を表す単文になる。これを **否定節** と呼ぶ。否定節も他の単文と並べて文を構成したり、他の否定節に入れ子にしたりできる。  

`nou`を **否定開始** と呼び、グロスは`{NEG`とする。また、`noi`を **否定終止** と呼び、グロスは`}NEG`とする。
...
`nou`は、直後の句を否定する前置単項演算子である。これを **単独否定** と呼び、グロスは`/NEG`とする。

単独否定が単文の前にあれば、その単文が表す論理式が、前置詞や関係詞、述語の前にあればそれが形成する述語句や名詞句が表す論理式が、それぞれその否定に置き換わる。単独否定が述語句や名詞句を否定するとき、その述語ポインタや変数は変化させない。

ちなみに単独否定が限定詞の前に置くこともできるが、これは空積の否定を表すのであまり使いどころがない。また、単独否定を二つ重ねることもできるが、これは二重否定なのでナンセンスである（古典論理に従い、二重否定を除去しても意味が変わらない意味論を採用する）

また、否定節同様、否定される範囲でのみ使用される変数は否定の中で存在量化され、外側でも使用される変数は否定の外で量化される。
