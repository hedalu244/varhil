...
**限定詞** は変数を表現するトークンである。
限定詞は、孤立限定詞`DET`、新規限定詞`DET自然数+`、継承限定詞`DET自然数`、終了限定詞`DET自然数-`の4種類がある。文中に出現した限定詞と変数の対応は以下のように決定される。  

+ 孤立限定詞`DET` は出現毎に他のどの限定詞の出現とも異なる変数を指す。
+ 新規限定詞`DET自然数+` は、それまでに出現したどの限定詞とも異なる変数を指す。
+ 継承限定詞`DET自然数` と 終了限定詞`DET自然数-` の形は、同じ番号の限定詞の、一つ前の出現と同じ変数を指す。
+ 終了限定詞`DET自然数+` は、それ以後に出現するどの限定詞とも異なる変数を指す。
