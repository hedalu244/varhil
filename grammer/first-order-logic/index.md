...
ある特定の格を **無標格** とする。無標格を表す関係詞が二番目の引数に不定名詞を取っている場合（ある述語句が無標格関係詞と不定名詞に挟まれている場合）、さらにその名詞句が単文になっていないとき、その関係詞と不定名詞は省略できる。つまり、無標格を取る述語句は単独で名詞句になれる。

これを読むときは、名詞句であるべきところ（前置詞の第一引数や関係詞の第二引数）が述語句であったとき（またその時のみ）に、無標格と不定名詞（つまりその場限りの変数）を補って読む。
...
**単独否定語** `/!` は、直後の句を否定する否定節を作る前置単項演算子である。単文の前にあればその単文の終わりまで、前置詞や関係詞の前にあればそれが形成する述語句や名詞句の終わりまで、述語の前にあればその述語のみを否定する。定名詞、不定名詞の前にあった場合はそれのみを否定するが、これは空積の否定なのでナンセンスである。また、単独否定語を二つ重ねることもできるが、二重否定なのでナンセンスである（古典論理に従い、二重否定を除去しても意味が変わらない意味論を採用する）
