...
冠詞は `//[格]` の形で表されるトークンである。冠詞は直後に述語句、その後ろに名詞句を取り、名詞句を成す。そして述語句の主述語の、冠詞の格に、名詞句の主変数を割り当てる。
...
量化の位置を変えるために、名詞句（特に変項）を他の単文の前に置いて連言を作ることができる。これを否定部の外等に置くことで、量化を調整することができる。つまり、単文が最後に来ない限り、単文と名詞句を好きな順に並べて連言を作ることができる。
